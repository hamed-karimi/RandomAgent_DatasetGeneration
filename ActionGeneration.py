...
def generate_action():
    ...
    for episode in range(params.EPISODE_NUM):
        ...
        rand_indices = torch.randperm(goal_selection_time.shape[0])
        while rand_indices.shape[0] < params.STEPS_NUM:
            back_up_indices = torch.randperm(goal_selection_time.shape[0])
